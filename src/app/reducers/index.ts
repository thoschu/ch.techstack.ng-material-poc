...
export const reducer: ActionReducer<AppState>= createReducer(
  ...
  on(
    appActions.loadApps,
    (state: AppState, action: TypedAction<'[App] Load Apps'> & {type: '[App] Load Apps'}): AppState => {
  ...
  on(
    ...
    (state: AppState, action: {app: string | Users} & TypedAction<'[App] Load Apps Config'> & {type: '[App] Load Apps Config'}): AppState => {
      console.log('reducer:loadApps');
      console.dir(action);
      console.log(state);
...
export const metaReducers: MetaReducer<State>[] = isDevMode() ? [loggerDev] : [loggerProd]; // Runs before normal reducers
