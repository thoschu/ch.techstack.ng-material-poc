...
export const reducers: ActionReducer<AdminState>= createReducer(
  initialAdminState,
  on(adminActions.setAdminAction,
    (state: AdminState, action: { admin: Admin<number>} & TypedAction<'[Admin Page] SetAdmin Action'> & {type: '[Admin Page] SetAdmin Action'}): AdminState => {
    console.log(state);
    return {
        admin: action.admin
      };
  ...
  on(adminActions.deleteAdminAction,
