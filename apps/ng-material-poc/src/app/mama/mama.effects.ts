  ...
  constructor(private readonly actions$: Actions) {
    ...
    this.actions$
      ...
      .pipe(
        tap(
          (
            action: { mama: string } & TypedAction<'[Mama] Load Mamas Success'>
          ): void => {
            console.log(action.type);
    ...
    this.effectName$ = createEffect(
      () =>
        this.actions$.pipe(ofType(mamaActions.loadMamasDelete)).pipe(
          tap((action: TypedAction<'[Mama] Load Mamas Delete'>): void => {
            console.log(action);
            console.log(action.type);
            // ToDo
          })
